...
#if USE_H264BSF
            av_bitstream_filter_filter(h264bsfc, in_stream->codec, nullptr, &pkt.data, &pkt.size, pkt.data, pkt.size, 0);
#endif
        }
        //ret = av_write_frame(ofmt_ctx, &pkt);
        ret = av_interleaved_write_frame(ofmt_ctx, &pkt);
